...
y_probability=y_predict
y_predict=(y_predict>0.5)

import numpy as np
from sklearn.metrics import confusion_matrix, classification_report
cm1=confusion_matrix(y_test,y_predict)
np.savetxt("confusion_matrix(original).csv", cm1, delimiter=",")
import pathlib

report1=classification_report(y_test,y_predict,digits=5)
pathlib.Path("report(original).txt").write_text(f"original:{report1}")
...
lw=2
auc1 = roc_auc_score(y_test.values, y_probability)
# calculate roc curve
fpr, tpr, thresholds = roc_curve(y_test.values, y_probability)
plt.figure()
plt.plot(fpr, tpr, color='darkorange', lw=1, label='ROC curve (area = %0.5f)' % auc1)
plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Receiver operating characteristic(orignial data)')
plt.legend(loc="lower right")
plt.savefig('original_roc.jpg')

plt.show()
yvalue=y_test.values
from pandas.tools.plotting import table

df=pd.DataFrame(data=cm1, columns = ["predicted missed playoffs", "predicted making playoffs"])
...
plt.savefig('confusion(original).png', transparent=True)
import PIL
import PIL.Image
...
def main():
    image = text_image('report(original).txt')
    image.show()
    image.save('classification_report(original).png')
...
ann_viz(classifier, title="ANN Structure");
