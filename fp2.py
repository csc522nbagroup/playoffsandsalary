# -*- coding: utf-8 -*-
"""
Created on Sun Dec  9 11:57:14 2018

@author: shaohan with changes by chris
"""
...
cm=confusion_matrix(y_test,y_predict01)
# cm2=confusion_matrix(y_test2,y_predict2)

print(classification_report(y_test,y_predict01))
...
plt.plot(fpr, tpr, color='darkorange',
         lw=lw, label='ANN curve (area = %0.2f)' % roc_auc) ###假正率为横坐标，真正率为纵坐标做曲线
...
mdl = LogisticRegression() # SVC(probability=True,kernel='linear'),GradientBoostingClassifier()]
mdl.fit(x_train, y_train) # train the model
y_pred=mdl.predict(x_test) # predict the test data
fpr, tpr, thresholds = metrics.roc_curve(y_test, mdl.predict_proba(x_test)[:,1])
auc = metrics.roc_auc_score(y_test,mdl.predict(x_test))
...
mdl = SVC(probability=True,kernel='linear')
mdl.fit(x_train, y_train) # train the model
y_pred=mdl.predict(x_test) # predict the test data
fpr, tpr, thresholds = metrics.roc_curve(y_test, mdl.predict_proba(x_test)[:,1])
auc = metrics.roc_auc_score(y_test,mdl.predict(x_test))
plt.plot(fpr, tpr, label='SVC linear kernel (auc = %0.2f)' % (auc))
    
mdl = SVC(probability=True,kernel='rbf')
mdl.fit(x_train, y_train) # train the model
y_pred=mdl.predict(x_test) # predict the test data
fpr, tpr, thresholds = metrics.roc_curve(y_test, mdl.predict_proba(x_test)[:,1])
auc = metrics.roc_auc_score(y_test,mdl.predict(x_test))
plt.plot(fpr, tpr, label='SVC rbf kernel (auc = %0.2f)' % (auc))
